...
RenderTexture::RenderTexture(ID3D11Device* device, int ltextureWidth, int ltextureHeight, float screenNear, float screenFar)
{
	...
	shaderResourceViewDesc.Texture2D.MipLevels = 1;
	// Create the shader resource view.
	result = device->CreateShaderResourceView(depthStencilBuffer, &shaderResourceViewDesc, &shaderShadowResourceView);
...
ID3D11ShaderResourceView* RenderTexture::getDepthResourceView()
{
	return shaderShadowResourceView;
}